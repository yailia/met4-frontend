...
describe('loadConfig', () => {
  ...
  it('applies defaults for optional vars', () => {
    const cfg = loadConfig({ RESEND_KEY: 're_test', FRONTEND_ORIGIN: 'https://met4.ru' } as NodeJS.ProcessEnv);
    expect(cfg.dbUrl).toBe('file:./db.sqlite');
    expect(cfg.leadInbox).toBe('bolkunatz@gmail.com');
    expect(cfg.port).toBe(3001);
  });

  it('throws when RESEND_KEY is missing', () => {
    expect(() => loadConfig({ FRONTEND_ORIGIN: 'https://met4.ru' } as NodeJS.ProcessEnv)).toThrow(/RESEND_KEY/);
  });

  it('throws when FRONTEND_ORIGIN is missing', () => {
    expect(() => loadConfig({ RESEND_KEY: 're_test' } as NodeJS.ProcessEnv)).toThrow(/FRONTEND_ORIGIN/);
  });
});
